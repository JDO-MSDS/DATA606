...
tv_hours_df <- yrbss %>%
  transmute(success = if_else(hours_tv_per_school_day %in% c("do not watch","<1","1","2"), "yes", "no")) %>%
  drop_na() %>%
  mutate(success = factor(success, levels = c("no","yes")))


ci_tv_low <- tv_hours_df %>%
  specify(response = success, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
...
sleep_hours_df <- yrbss %>%
  transmute(success = if_else(school_night_hours_sleep %in% c("8","9","10+"), "yes", "no")) %>%
  drop_na() %>%
  mutate(success = factor(success, levels = c("no","yes")))


ci_sleep_enough <- sleep_hours_df %>%
  specify(response = success, success = "yes") %>%
  generate(reps = 1000, type = "bootstrap") %>%
  calculate(stat = "prop") %>%
  get_ci(level = 0.95, type = "percentile") %>%
